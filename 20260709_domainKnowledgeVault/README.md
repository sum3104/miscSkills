...
- **commit ガード + ローカル instructions（除外運用時）**: superpowers の
  ...
  ローカル専用 instructions ファイル**（Copilot:
  `.github/instructions/dkv-local.instructions.md`、Claude Code:
  `CLAUDE.local.md`、Cursor: `.cursor/rules/dkv-local.mdc`）を作成し、
  「除外パス配下の commit 指示はスキップ / `git add -f` 禁止」という
  ガード文を書き込みます。ガードを共有の instructions に書くとそれ自体が
  痕跡になり、チームメンバーのエージェントまで commit をスキップして
  しまうため、必ずローカルファイル側に置きます。同じ理由で、共有ファイル名
  `.github/copilot-instructions.md` を先取り作成することもしません
  （後日チームが同名ファイルを追加すると pull が untracked 衝突で失敗
  するため）。
...
- 自動起動は instructions ファイルの指示に依存します。エージェントが指示を
  ...
  ください（各 SKILL.md の description にもトリガーを記載済み）。
